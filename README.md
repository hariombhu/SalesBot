...
## Overview
This project demonstrates a simple business website with an AI-powered chatbot
...
│
└── docs/
    └── architecture.png (optional, for hackathon submission)
...
- **To test (demo script):**
  1. Open `index.html` and click the chat icon.
  2. Ask a demo: "I want a demo" → bot will ask for your name → provide name → provide an email like `alice@example.com` → bot shows simulated follow-up after a moment and lead score increases.
  3. Ask pricing: "What is your pricing?" → lead score increases and bot offers a quote.
  4. Express frustration: "This feature is not working, I'm frustrated" → the bot uses an empathetic tone and offers to escalate to a human.
  5. For judges: type `why this answer` to show the detection details and lead score.
...
- Demo steps:
  1. Open the chat and click 📎 to upload an image of a product or click a product's **Buy** button.
  2. If matched, the bot will show price and offer to buy; if unmatched, the bot will ask if you want to become a seller or request a price estimate.
  3. The upload is optional and stored locally; you can show the `uploads/` folder contents to judges when the demo server is running.
